  ...
  conn, paste0(
    "SELECT  date(fecha_inicio) AS fecha_inicio, categoria_delito 
     ...
     WHERE 
       ...
       AND EXTRACT(MONTH FROM fecha_inicio) <= '", ultimo_mes_completo ,"'
  ...
  conn, paste0(
    "SELECT date(fechainicio) AS fecha_inicio, cve_delito AS categoria_delito
     ...
     WHERE 
       ...
       AND EXTRACT(MONTH FROM fechainicio) <= '", ultimo_mes_completo, "'"
  )
) %>% as_tibble()
...
generar_grafico_delito <- function(nombre_delito, datos) {
  ...
  resumen <- datos_filtrados %>%
    ...
    mutate(
      dias_acumulados = map_dbl(year, ~{
        fecha_inicio_anio <- as.Date(paste0(.x, "-01-01"))
        fecha_fin_anio <- ceiling_date(as.Date(paste0(.x, "-", ultimo_mes_completo, "-01")), "month") - days(1)
        dias <- as.numeric(fecha_fin_anio - fecha_inicio_anio + 1)
        return(dias)
      }),
      promedio_diario = total_registros / dias_acumulados
    )
  ...
  ggsave(
    filename = here("salidas", glue("grafico_acum_{nombre_archivo}.png")),
    plot = p,
    width = 20,
    height = 10,
    dpi = 100
